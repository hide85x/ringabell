export default defineEventHandler(async (event) => {
  ...
  // 3. Event-level: Ratownik
  const ratownikRow = await db
    .prepare(`SELECT COUNT(*) AS count FROM assignments WHERE event_id = ? AND type = 'event' AND role = 'Ratownik'`)
    .bind(id)
    .first() as { count: number } | null
  if ((ratownikRow?.count ?? 0) === 0) {
    errors.push('Brak ratownika na gali')
  }

  // 4. Event-level: Konferansjer
  const konferansjerRow = await db
    .prepare(`SELECT COUNT(*) AS count FROM assignments WHERE event_id = ? AND type = 'event' AND role = 'Konferansjer'`)
    .bind(id)
    .first() as { count: number } | null
  if ((konferansjerRow?.count ?? 0) === 0) {
    errors.push('Brak konferansjera na gali')
  }

  // 5. Date conflicts — two separate queries to avoid UNION issues in D1

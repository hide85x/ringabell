export default defineEventHandler(async (event) => {
  ...
  let requirements: { id: string; fightId: string; role: string; count: number }[] = []
  if (fightIds.length > 0) {
    const placeholders = fightIds.map(() => '?').join(', ')
    const { results } = await db
      .prepare(`SELECT id, fight_id AS fightId, role, count FROM fight_requirements WHERE fight_id IN (${placeholders})`)
      .bind(...fightIds)
      .all() as { results: { id: string; fightId: string; role: string; count: number }[] }
    requirements = results
  }

  // 4. Fight assignments
  let fightAssignments: { id: string; fightId: string; personId: string; personName: string; role: string }[] = []
  if (fightIds.length > 0) {
    const placeholders = fightIds.map(() => '?').join(', ')
    const { results } = await db
      .prepare(
        `SELECT a.id, a.fight_id AS fightId, a.person_id AS personId, p.name AS personName, a.role
         FROM assignments a
         JOIN persons p ON p.id = a.person_id
         WHERE a.type = 'fight' AND a.fight_id IN (${placeholders})`,
      )
      .bind(...fightIds)
      .all() as { results: { id: string; fightId: string; personId: string; personName: string; role: string }[] }

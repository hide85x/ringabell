...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "główne źródła bólu"
      decision: "konflikty terminów + niekompletna obsada + brak jednego miejsca widoczności"
    - topic: "persona główna"
      decision: "Manager / promotor — planuje galę, ponosi ryzyko błędu"
    - topic: "insight"
      decision: "koszt błędu rósł wraz ze skalą — Excel/telefon przestał wystarczać"
    - topic: "auth"
      decision: "OAuth (Google/Facebook) — brak haseł w systemie"
    - topic: "model ról"
      decision: "3 role: Admin (konta + słowniki), Manager (planowanie gal), Personel (tylko podgląd)"
    - topic: "powiadomienia"
      decision: "email — tylko przy tworzeniu gali (jeden email z datą, miejscem, rolą); powiadomienia przy zmianach = v2"
    - topic: "MVP scope"
      decision: "scope down: wycięto powiadomienia przy każdej zmianie; kalendarz zostaje w MVP"
    - topic: "mvp_weeks"
      decision: "4 tygodnie; częściowo w godzinach pracy; brak twardego deadline'u"
    - topic: "non-goals"
      decision: "brak powiadomień przy zmianach, brak raportowania, brak integracji kalendarzy zewnętrznych"
  frs_drafted: 15
...
Logowanie przez OAuth (Google / Facebook) — brak haseł przechowywanych w systemie.
...
Niezalogowany użytkownik nie widzi żadnych danych — każda trasa jest chroniona.
...
- FR-004: Manager może dodawać/edytować/dezaktywować osoby z bazy personelu (dezaktywacja zamiast twardego usunięcia). Priority: must-have
  > Socrates: Twarde usunięcie niszczy historię przypisań do przeszłych gal. Decyzja: dezaktywacja — osoba znika z list wyboru, historia zostaje.
...
- Wynik walidacji po przypisaniu osoby widoczny poniżej 1 sekundy — bez oczekiwania na serwer.
- Aplikacja działa na desktopie i urządzeniach mobilnych (responsive) — manager planuje przy komputerze, personel sprawdza harmonogram na telefonie.
...
- Każda chroniona trasa jest niedostępna dla niezalogowanego użytkownika.
...
- **Brak integracji z zewnętrznymi kalendarzami** — brak synchronizacji z Google Calendar, iCal ani innymi systemami. Kalendarz w aplikacji jest wewnętrzny.

## Timeline acknowledgment
Acknowledged on 2026-05-20: 4-week MVP requires sustained dedication; user accepted after scope-down (powiadomienia przy zmianach wypchnięte do v2).
